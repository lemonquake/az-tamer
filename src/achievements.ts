...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  { id: 'split', icon: '🪢', name: 'Crossroads', desc: 'Branch a Guardian into a Split Evolution (4th form).', check: p => allGuardians(p).some(g => stageIdx(g.species.stage) >= 4) },
  { id: 'terra', icon: '🌐', name: 'Worldforged', desc: 'Ascend a Guardian to a Terra Evolution or beyond.', check: p => allGuardians(p).some(g => stageIdx(g.species.stage) >= 6) },
  { id: 'legend', icon: '🌟', name: 'Keeper of Legends', desc: 'Raise or ascend a Guardian to an Aether form — the rank of the Big Three\'s Nine.', check: p => allGuardians(p).some(g => stageIdx(g.species.stage) >= 8) },
